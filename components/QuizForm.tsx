...
export default function QuizForm({ lessonId, initialData, isEdit = false }: QuizFormProps) {
  ...
  return (
    <div className="mx-auto max-w-5xl p-4 sm:p-6">
      {/* Header banner */}
      <div className="mb-6 rounded-[4px] border border-ink bg-sheet p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <div className="rounded-[4px] bg-white p-2 ring-1 ring-ink">
              <Edit3 className="h-5 w-5 text-ink" />
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden rounded-[4px] bg-white px-3 py-2 text-sm font-medium text-ink ring-1 ring-ink sm:flex sm:flex-col">
              <span className="inline-flex items-center gap-2">
                <LayoutList className="h-4 w-4 text-ink" />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="border-rule">
          <CardHeader className="border-b border-rule">
            <CardTitle className="flex items-center gap-2 text-lg">
              <Info className="h-5 w-5 text-graphite" />
              General Settings
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-5 pt-6">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                />
                <p className="mt-1 text-xs text-graphite">A short, descriptive title for the quiz.</p>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <Textarea
                  ...
                  className="resize-y focus-visible:ring-ink"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <Input
                    ...
                    className="pr-10 focus-visible:ring-ink"
                    min={0}
                  />
                  <Timer className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-graphite" />
                </div>
                <p className="mt-1 text-xs text-graphite">Leave empty for no time limit.</p>
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  className="focus-visible:ring-ink"
                />
                <p className="mt-1 text-xs text-graphite">How many times a student can attempt this quiz.</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {formData.questions.map((question, questionIndex) => {
          ...
          return (
            <Card
              key={questionIndex}
              className="overflow-hidden border-rule transition-all"
            >
              <CardHeader className="flex flex-row items-center justify-between border-b border-rule py-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-[4px] bg-[#edf2f5] text-ink ring-1 ring-ink">
                    {questionIndex + 1}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 rounded-[4px] bg-[#edf2f5] px-2 py-1 text-xs font-medium text-ink ring-1 ring-ink">
                  ...
                  {formData.questions.length > 1 && (
                    <Button
                      ...
                      className="cursor-pointer border-mark text-mark hover:bg-[#fdf3f2] hover:text-mark"
                    >
              ...
              <CardContent className="space-y-5 pt-6">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      <div className="relative sm:flex-1">
                        <Input
                          ...
                          className="pr-10 focus-visible:ring-ink"
                        />
                        <ImageIcon className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-graphite" />
                      </div>

                      <div className="flex items-center gap-2">
                        <label className="inline-flex cursor-pointer items-center rounded-[4px] border border-rule px-3 py-2 text-sm font-medium text-ink hover:bg-[#edf2f5]">
                        ...
                        {question.imageUrl ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="cursor-pointer inline-flex items-center rounded-[4px] border border-rule px-3 py-2 text-sm text-ink hover:bg-[#edf2f5]"
                          >
                    ...
                    <p className="mt-1 text-xs text-graphite">PNG, JPG, or WEBP up to 5MB.</p>
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="focus-visible:ring-ink"
                    />
                    <p className="mt-1 text-xs text-graphite">Default is 1 point.</p>
                  </div>
                </div>
                ...
                <div>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span title="Select one correct option">
                      <HelpCircle className="h-4 w-4 text-graphite" />
                    </span>
                  </div>
                  ...
                  <div className="space-y-2">
                    {question.options.map((option, optionIndex) => {
                      ...
                      return (
                        <div
                          key={optionIndex}
                          className={`flex items-start gap-3 rounded-[4px] border p-4 transition-colors ${ isActive ? 'border-ink bg-[#edf2f5]/60' : 'border-rule hover:border-rule' }`}
                        >
                          <input
                            type="radio"
                          ...
                          <div className="grid w-full gap-2 sm:grid-cols-2">
                            <div className="sm:col-span-2">
                              <Label className="mb-1 block text-xs text-graphite">
                            ...
                            <div className="sm:col-span-2">
                              <Label className="mb-1 block text-xs text-graphite">Option Image (optional)</Label>
                ...
                <div className="flex justify-end pt-2">
                  <Button
                    ...
                    variant="outline"
                    onClick={addQuestion}
                    className="cursor-pointer border-rule hover:border-rule hover:bg-[#edf2f5]"
                  >
        ...
        <div className="flex justify-center">
          <Button
            ...
            variant="outline"
            onClick={addQuestion}
            className="cursor-pointer mt-2 border-rule hover:border-rule hover:bg-[#edf2f5]"
          >
            <Plus className="mr-2 h-4 w-4" />
            Add Question
          </Button>
        ...
        <div className="sticky bottom-0 z-10 -mx-4 border-t border-rule bg-white/85 px-4 py-4 backdrop-blur supports-[backdrop-filter]:bg-white/60 sm:mx-0 sm:rounded-[4px] sm:border sm:px-6">
          <div className="flex flex-col items-center justify-between gap-3 sm:flex-row">
            <div className="text-sm text-graphite">
              <span className="font-medium text-ink">{formData.questions.length}</span> question
              {formData.questions.length === 1 ? '' : 's'} •{' '}
              <span className="font-medium text-ink">{totalPoints}</span> total points
              {uploading ? <span className="ml-2 text-ink">• Uploading...</span> : null}
            </div>
            ...
            <div className="flex w-full items-center justify-end gap-2 sm:w-auto">
              <Button
                ...
                variant="outline"
                onClick={() => router.back()}
                className="cursor-pointer border-rule hover:bg-[#edf2f5]"
                disabled={uploading}
              >
                Cancel
              </Button>
              <Button
                ...
                className="cursor-pointer bg-ink hover:bg-ink focus-visible:ring-ink"
              >
