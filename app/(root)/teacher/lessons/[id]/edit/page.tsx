    ...
    <div className="sheet-page mx-auto w-full max-w-5xl py-10 bg-[#edf2f5]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-ink">Edit Lesson</h1>
          <p className="text-graphite mt-2">Update lesson details and settings</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[4px] border border-rule overflow-hidden">
          <div className="px-8 py-6 border-b border-rule">
            <h2 className="text-xl font-semibold text-ink">Lesson Information</h2>
          </div>
          ...
          <div className="p-8 space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  className="flex-1 px-4 py-2 border border-rule rounded-[4px] focus:ring-2 focus:ring-ink focus:border-ink transition-colors"
                  placeholder="Add a tag"
                />
                <button
                  type="button"
                  onClick={addTag}
                  className="cursor-pointer px-4 py-2 bg-ink text-white rounded-[4px] hover:bg-ink transition-colors"
                >
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-[#edf2f5] text-ink text-sm font-medium rounded-full"
                  >
                    {tag}
                    <button
                      type="button"
                      onClick={() => removeTag(tag)}
                      className="cursor-pointer ml-1 hover:bg-[#01243a] rounded-full p-0.5"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </span>
            ...
            {/* Error Message */}
            {error && (
              <div className="bg-[#fdf3f2] border border-mark text-mark px-4 py-3 rounded-[4px]">
                {error}
              </div>
            )}
          </div>
