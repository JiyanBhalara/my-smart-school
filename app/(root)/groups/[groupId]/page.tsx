  ...
  return (
    <div className="min-h-screen bg-[#edf2f5] pt-20 px-4 lg:px-6 mb-7">
      <div className="max-w-5xl mx-auto h-[calc(100vh-6rem)] bg-white rounded-[4px] border border-rule overflow-hidden flex flex-col">
        ...
        <header className="flex items-center justify-between p-4 lg:p-6 border-b border-rule bg-white">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="cursor-pointer flex items-center space-x-2 text-graphite hover:text-ink px-3 py-2 rounded-[4px] hover:bg-[#edf2f5] transition-colors duration-200"
            >
        ...
        {/* Pinned Message */}
        {group.pinnedMessage && (
          <div className="bg-[#edf2f5] border-b border-mark p-4">
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-3 flex-1 min-w-0">
                <svg className="w-4 h-4 text-mark mt-1 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
              ...
              {isAdmin && (
                <button
                  ...
                  className="cursor-pointer text-mark hover:text-mark p-1 rounded transition-colors"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              return (
                <div key={message.id} className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                  <div className="relative group">
                    <div className={`max-w-xs lg:max-w-lg xl:max-w-xl px-4 py-3 rounded-[4px] ${ isCurrentUser ? 'bg-ink text-white' : 'border border-rule bg-sheet text-ink' }`}>
                        ...
                        <div className="mt-3">
                          {message.fileType?.startsWith('image/') ? (
                            <div className="space-y-2">
                              ...
                              />
                              {message.fileName && (
                                <p className={`text-xs truncate ${isCurrentUser ? 'text-ink' : 'text-graphite'}`}>
                                  {message.fileName}
                                </p>
                              )}
                            </div>
                            ...
                            <a
                              ...
                              className={`flex items-center space-x-3 p-3 rounded-[4px] border transition-colors ${ isCurrentUser ? 'bg-ink border-ink hover:bg-ink' : 'bg-[#edf2f5] border-rule hover:bg-[#edf2f5]' }`}
                            >
                              <svg className={`w-6 h-6 ${isCurrentUser ? 'text-ink' : 'text-graphite'}`} fill="currentColor" viewBox="0 0 24 24">
                                <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z" />
                              </svg>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate">{message.fileName}</p>
                                {message.fileSize && (
                                  <p className={`text-xs ${isCurrentUser ? 'text-ink' : 'text-graphite'}`}>
                      ...
                      <p className={`text-xs mt-2 text-right ${ isCurrentUser ? 'text-ink' : 'text-graphite' }`}>
                    ...
                    {isAdmin && !isCurrentUser && (
                      <button
                        ...
                        className="cursor-pointer absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-[#edf2f5]"
                      >
                    ...
                    {showPinOptions === message.id && (
                      <div className="absolute left-0 top-full mt-1 bg-white border border-rule rounded-[4px] z-10">
                        <button
                          onClick={() => handlePinMessage(message.id)}
                          className="cursor-pointer flex items-center space-x-2 px-3 py-2 text-sm text-ink hover:bg-[#edf2f5] rounded-[4px]"
                        >
          ...
          <form onSubmit={handleSendMessage} className="flex items-end space-x-3">
            <div className="flex-1">
              <textarea
                ...
                className="w-full p-3 lg:p-4 border border-rule rounded-[4px] focus:ring-2 focus:ring-ink focus:border-transparent resize-none text-ink placeholder-slate-400 transition-colors duration-200 text-sm lg:text-base"
                rows={1}
                style={{ 
                  minHeight: '48px',
                  maxHeight: '120px',
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                className="cursor-pointer flex items-center justify-center w-12 h-12 bg-ink text-white rounded-[4px] hover:bg-ink focus:outline-none focus:ring-2 focus:ring-ink disabled:opacity-50 transition-colors duration-200"
              >
                {sending ? (
      ...
      {showMembers && (
        <>
          ...
          <div className="fixed right-0 top-20 bottom-0 w-80 bg-white border-l border-rule z-50 overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-3">
                {group.members.map((member) => (
                  <div key={member.user.id} className="flex items-center space-x-3 p-2 rounded-[4px] hover:bg-[#edf2f5]">
                    {member.user.image ? (
                      <Image
